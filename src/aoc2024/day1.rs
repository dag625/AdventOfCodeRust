...
pub fn solve_1(input_dir: &Path) -> Result<()> {
    solve(crate::aoc2024::day1::solve_1_impl, input_dir)
}

pub fn solve_2(input_dir: &Path) -> Result<()> {
    solve(crate::aoc2024::day1::solve_2_impl, input_dir)
}

fn solve<R: Display>(func: fn(& mut (Vec<i32>, Vec<i32>)) -> R, input_dir: &Path) -> Result<()> {
    let res = func(& mut get_input(input_dir)?);
    println!("\t{}", res);
    Ok(())
}

fn get_input(input_dir: &Path) -> Result<(Vec<i32>, Vec<i32>)> {
    let lines = utilities::get_input_lines(input_dir.join("2024").join("day_1_input.txt"))?;
